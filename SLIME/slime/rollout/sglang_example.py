...
class GenerateState(metaclass=SingletonMeta):
    ...
    def __init__(self, args):
        ...
        self.semaphore = asyncio.Semaphore(
            args.sglang_server_concurrency * args.rollout_num_gpus // args.rollout_num_gpus_per_engine      # 总共 ray engine actor 的数量 * 每个 actor 接受的并发数量
        )
    ...
    def submit_generate_tasks(self, samples: list[list[Sample]]):
        '''异步的方式创建采样任务，以batch:[prompt:[次数,...], ...]为输入， 调用generate_and_rm_group'''
...
async def generate_rollout_async(args, rollout_id: int, data_buffer) -> list[list[Sample]]:
    ...
    采样过程实际调用函数 generate_and_rm_group 。在整个采样过程中，会经过 dynamic_filter 与 over_sampling_filter 两个过滤
    ...
    # target_data_size 是总共进行rollout的数量，支持过采样
    ...
    while len(data) < target_data_size:
        # 获取 target_data_size 个样本，由于下面每采样得到1个，就会减少减少一个remaining_batch_size，所以这个会尽可能保证任务数量在 target_data_size 之上。
        # 然而其实实际只需要 target_data_size 个就够了，所以额外生成的
        ...
        # 每一个done，代表一个prompt采样的 K 个样本都已经得到。
        done, state.pendings = await asyncio.wait(state.pendings, return_when=asyncio.FIRST_COMPLETED)
        
        # 遍历done，加入所有已经完成的数据到data中。
        for task in done:
    ...
    data = sorted(data, key=lambda group: group[0].index)       # !index是sample时prompt复制的id，为啥这里要按照index排列?

    # reset the global state to prevent effects on the next rollout or eval.
    # 这里是为了reset state类中的全局变量..., 应该是架构问题，不应该这么设计。
    state.reset()
    return data
...
def generate_rollout(args, rollout_id, data_buffer, evaluation=False):
    """由Buffer.generate调用，在Buffer单Agent中执行，concurrency<-1
    An example to implement the generate_rollout function for an rule based rm rollout generation.
